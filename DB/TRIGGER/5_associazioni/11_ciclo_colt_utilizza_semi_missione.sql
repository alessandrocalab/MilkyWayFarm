...
CREATE TRIGGER TRG_CICLO_COLTIVAZIONE_UTILIZZA_SEMI_MISSIONE_SEMI_DISP
...
CREATE TRIGGER TRG_CICLO_COLT_UTILIZZA_SEMI_MISSIONE_SEMI_COMP
BEFORE INSERT OR UPDATE ON CICLO_COLT_UTILIZZA_SEMI_MISSIONE
FOR EACH ROW 
...
BEGIN
    ...
    WHERE T.NOME_SEMI=:NEW.NOME_SEMI;
...
--sono arrivati dopo l'inizio
--del ciclo coltivazione

CREATE TRIGGER TRG_CICLO_COLT_UTILIZZA_SEMI_MISSIONE_SEMI_NON_ARRIVATI
BEFORE INSERT OR UPDATE ON CICLO_COLT_UTILIZZA_SEMI_MISSIONE
FOR EACH ROW 
...
BEGIN
    ...
    IF :NEW.DATA_ARRIVO_SEMI > :NEW.DATA_INIZIO
        THEN 
            RAISE SEMI_INCOMPATIBILI;
    END IF;
