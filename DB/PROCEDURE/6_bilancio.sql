...
BEGIN 
    ...
    --1: CALCOLO I CONSUMI ANIMALI A PARTIRE DAL PRIMO 
    ...
    FOR RW IN(
        ...
        JOIN TIPO_PRODOTTO UM 
        ON CONSUMI_TOTALI.NOME_PRODOTTO = UM.NOME_PRODOTTO
    ) LOOP

        DBMS_OUTPUT.PUT_LINE('nome prodotto: '|| RW.NOME_PRODOTTO);
        DBMS_OUTPUT.PUT_LINE('quantita consumata: '|| RW.QUANTITA_CONSUMATA|| RW.CODICE_UNITA);
        DBMS_OUTPUT.PUT_LINE('quantita prodotta: '|| RW.QUANTITA_PRODOTTA|| RW.CODICE_UNITA);
        DBMS_OUTPUT.PUT_LINE('bilancio: '|| RW.BILANCIO|| RW.CODICE_UNITA);
